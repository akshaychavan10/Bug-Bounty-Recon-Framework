...
def find_robots_txt(target_domain):
    """
    Perform Google Dork queries to find robots.txt files.
    Save the results in a structured format (JSON) inside `outputs/target_domain/robots_txt.json`.
    """
    # Define Google Dork queries with the target domain
    queries = [
        f'site:{target_domain} inurl:"robots.txt"',
        f'site:{target_domain} filetype:txt "User-agent:"',
        f'site:{target_domain} intitle:"index of" "robots.txt"'
    ]
    ...
    # Output file path
    output_file = f"{output_dir}/robots_txt.json"
